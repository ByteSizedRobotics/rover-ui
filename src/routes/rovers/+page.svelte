	...
	<div class="container mx-auto p-6 max-w-7xl">
		...
		<header class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
			...
			<div class="flex flex-col sm:flex-row gap-3">
				...
				<div class="flex rounded-lg overflow-hidden border border-blue-200">
					<button 
						class="px-4 py-2 text-sm font-medium transition-colors {filter === 'all' ? 'bg-blue-500 text-white' : 'bg-white text-blue-600 hover:bg-blue-50'}"
						onclick={() => filter = 'all'}
					>
						All
					</button>
					<button 
						class="px-4 py-2 text-sm font-medium transition-colors border-l border-blue-200 {filter === 'active' ? 'bg-blue-500 text-white' : 'bg-white text-blue-600 hover:bg-blue-50'}"
						onclick={() => filter = 'active'}
					>
						Active
					</button>
					<button 
						class="px-4 py-2 text-sm font-medium transition-colors border-l border-blue-200 {filter === 'inactive' ? 'bg-blue-500 text-white' : 'bg-white text-blue-600 hover:bg-blue-50'}"
				...
				<div class="relative">
					...
					<input 
						...
						class="border border-blue-200 rounded-lg pl-10 pr-4 py-2 w-full sm:w-64 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
					...
					<div class="p-6">
						<div class="flex items-start justify-between gap-3 mb-3">
							<div class="flex items-center gap-3 flex-1 min-w-0">
								<div class="px-2 py-1 rounded bg-blue-100 text-blue-700 text-xs font-mono">Rover-{rover.id}</div>
								<h2 class="text-lg font-bold text-blue-900 truncate">{rover.name}</h2>
							</div>
							<div class="text-xs text-blue-600 whitespace-nowrap">
		...
		{#if filteredRovers.length === 0}
			<div class="text-center py-16">
				<div class="bg-blue-50 border-2 border-dashed border-blue-200 rounded-2xl p-8 max-w-md mx-auto">
					<svg class="w-16 h-16 text-blue-300 mx-auto mb-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
						<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
					</svg>
					<h3 class="text-lg font-semibold text-blue-700 mb-2">No rovers match your filters</h3>
