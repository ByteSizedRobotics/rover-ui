...
export const POST: RequestHandler = async ({ request, params }) => {
    ...
    const { latitude, longitude, altitude, roll, pitch, yaw, temperature, voltage } = await request.json();

    if (!roverId) {
        return new Response(JSON.stringify({ error: 'Rover ID is required' }), { status: 400 });
    }
    if (!latitude || !longitude || altitude === undefined || roll === undefined || pitch === undefined || yaw === undefined || temperature === undefined || voltage === undefined) {
